...
public class ClientController {
	...
	public @ResponseBody ResponseEntity<List<Client>> allClients(
		...
		if (name == null) {
			...
			List<Client> clients = clientDao.findByNameContaining(name);
			return new ResponseEntity<List<Client>>(clientDao.findByNameContaining(name), HttpStatus.OK);
		}
	}

	@RequestMapping(value = "/clients", method = RequestMethod.POST)
	public ResponseEntity<Client> addClient(HttpServletRequest request, 
		...
		if (user != null && password != null) {
			try {
				...
				// TODO esto hay que cambiarlo por un mensaje de error!!
